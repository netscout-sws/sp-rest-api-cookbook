...
def get_mits(
    leader, api_key, alert_id, get_all_mits, max_page, page=1, matched_mits=[]
):
    ...
    if results.status_code != requests.codes.ok:
        print(
            "API request was not OK: {} {}".format(results.status_code, results.reason),
            file=sys.stderr,
        )
    ...
    if "next" in mits["links"]:
        next_page = int(parse_qs(urlparse(mits["links"]["next"]).query)["page"][0])
        if next_page > max_page:
            return matched_mits
        print(
            "Moving to page {} having found {} matching mitigations.".format(
                next_page, len(matched_mits)
            )
        )
        get_mits(
            leader, api_key, alert_id, get_all_mits, max_page, next_page, matched_mits
        )
...
def main():
    ...
    parser.add_argument("-k", "--apikey", dest="api_key", required=True, help="API key")
    parser.add_argument(
        "-p",
        "--maxpage",
        dest="max_page",
    ...
    mits = get_mits(
        args.leader, args.api_key, args.alert_id, args.get_all_mits, args.max_page
    )
