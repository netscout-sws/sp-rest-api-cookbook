...
def get_mitigations(leader, key, start, end, page=1, perPage=50):
    ...
    api_response = requests.get(
        URL, headers={"X-Arbux-APIToken": key}, verify=CERT_FILE
    )

    # check the API response
    if api_response.status_code != requests.codes.ok:
        print(
            "[ERROR] API responded with this error: "
            "{}\n(url: {})".format(api_response.reason, URL),
            file=stderr,
        )
        return []
    ...
    if "start" in data[-1]["attributes"]:
        ...
        oldest_mit_on_this_page = string_to_date(oldest_mit_on_this_page)

    if oldest_mit_on_this_page is None or oldest_mit_on_this_page > specified_start:
        #
        # parse out the last page from the links section
        #
        last_page = None
        if "links" in api_response and "last" in api_response["links"]:
            last_page = int(
                parse_qs(urlparse(api_response["links"]["last"]).query)["page"][0]
            )
...
def get_alerts_from_mits(mits):
    ...
    for mit in mits:
        ...
        alert_id = mit["relationships"]["alert"]["data"]["id"]
        if alert_id not in alerts:
            alerts[alert_id] = {}
            mit_start_time = string_to_date(mit["attributes"]["start"])

            alerts[alert_id]["mit_start_time"] = mit_start_time
            alerts[alert_id]["mit_type"] = mit["attributes"]["subtype"]
            alerts[alert_id]["started_by"] = mit["attributes"]["user"]
...
def print_report(alerts, mitigations, sp_leader, start, end):
    ...
    print("                 from:", start.strftime(TIMEFMT))
    print("                   to:", end.strftime(TIMEFMT))

    print(
        "Out of {} mitigations on {}, "
        "{} have associated alerts".format(
            len(mitigations), sp_leader, len(alerts.keys())
        )
    )
    ...
    for started_by in sorted(by_user_type_time):
        ...
        for mit_type in sorted(by_user_type_time[started_by]):
            ...
            for a_to_m_secs in sorted(by_user_type_time[started_by][mit_type]):
                print(
                    "{0:>20} | {1:<10} | {2:11} | {3}".format(
                        tmp_user,
                        tmp_mit_type,
                        a_to_m_secs,
                        ", ".join(by_user_type_time[started_by][mit_type][a_to_m_secs]),
                    )
                )
                tmp_mit_type = ""
                tmp_user = ""
...
def get_alert_start_time(leader, key, alert):
    ...
    if api_response.status_code != requests.codes.ok:
        print(
            "[WARNING] In retrieving information "
            "about alert {}, the API responded `{}'".format(api_response.reason, alert),
            file=stderr,
        )
        return None
    ...
    alert_start_time = string_to_date(api_response["data"]["attributes"]["start_time"])
...
def string_to_date(date_string):
    ...
    date_string = strptime(date_string, "%Y-%m-%dT%H:%M:%S")
    date_string = datetime.fromtimestamp(mktime(date_string))
    return date_string
...
if __name__ == "__main__":
    ...
    mitigations = get_mitigations(SP_LEADER, API_KEY, START_TIME, END_TIME)
    if not mitigations:
        exit('no mitigations found, exiting.')
    ...
    for alert in alerts:
        ...
        alerts[alert]["alert_to_mit_seconds"] = (
            alerts[alert]["mit_start_time"] - alerts[alert]["alert_start_time"]
        ).total_seconds()
