...
try:
    ...
    exit('missing dateutil module, install with: "pip install python-dateutil"')
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "ERROR: I couldn't parse the dates you provided ("
    ...
    print(
        "  Querying {} for {} raw flows records between {} and {}...".format(
            args.leader, args.num_records, start, end
        )
    )
    ...
    answer = requests.post(
        leaderurl,
        headers={
            "X-Arbux-APIToken": args.apikey,
            "Content-Type": "application/vnd.api+json",
            "Accept": "text/csv",
        },
        data=json.dumps(query),
        verify=args.certfile,
        stream=True,
    )
    query_end = datetime.now()
    with open(args.output_file, "wb") as f:
        for i, chunk in enumerate(answer.iter_content(chunk_size=262144)):
            print("  Writing chunk #{} to {}".format(i, args.output_file), end="\r")
            f.write(chunk)
    ...
    print("")
    print(
        "Query time: {0:10.1f} seconds".format(
            ((query_end - query_start).total_seconds())
        )
    )
    print(
        "Write time: {0:10.1f} seconds".format(
            ((write_end - query_end).total_seconds())
        )
    )
    print(
        "Total time: {0:10.1f} seconds".format(
