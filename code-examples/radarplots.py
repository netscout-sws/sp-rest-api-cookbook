...
try:
    ...
    exit('Cannot import matplotlib, install with "pip install matplotlib"')
try:
    import pandas
    import pandas as pd
except ImportError:
    exit('Cannot import pandas, install with "pip install pandas"')
...
def get_alerts(leader, apitoken, per_page, ongoing, include):
    ...
    url = "https://{}/api/sp/alerts/?perPage={}&{}".format(leader, per_page, FILTER)
    if ONGOING:
    ...
    if results.status_code != requests.codes.ok:
        print(
            "Results: {} ({})".format(results.reason, results.status_code),
            file=sys.stderr,
        )
        return None
...
def make_spiders(df):
    ...
    plt.subplots_adjust(
        left=0.125, bottom=0.1, right=0.9, top=0.9, wspace=0.6, hspace=0.2
    )
    ...
    row = 0
    for axkey in [str(m) for m in sorted([int(n) for n in df.keys()], reverse=True)]:
        # print ("{}: {}".format(
...
def get_radar_fractions(data):
    ...
    for alert_id in data:
        ...
        for key in data[alert_id].keys():
            frac_data[alert_id][key] = (
                float(data[alert_id][key]) / maxval[key]
            ) * 100.0
    return frac_data
...
if __name__ == "__main__":
    ...
    alerts = get_alerts(SP_LEADER, API_KEY, PER_PAGE, ONGOING, INCLUDE)

    if not alerts:
        print(
            "Did not retrieve any valid DoS "
    ...
    print(
        "Processing radar data for {} alerts".format(len(alerts["data"])),
        file=sys.stderr,
    )
